...
def carregar_planilha():
    try:
        ...
        workbook = load_workbook(arquivo)  # Carrega o arquivo para manipulação com openpyxl
        print(f"Planilha carregada: {arquivo}")
        return excel_file, abas, arquivo, workbook  # Retorna o caminho do arquivo e o objeto workbook
    ...
    if not dados_pag.empty:
        ...
        for i, row in dados_pag.iterrows():
            for j, column in enumerate(dados_pag.columns):
                label = ctk.CTkLabel(frame_lista, text=row[column], width=130, anchor="w", font=("Arial",10))
                label.grid(row=i+1, column=j, padx=10, pady=5)

                # Adicionando a possibilidade de editar
                label.bind("<Button-1>", lambda e, i=i, j=j: editar_celula(i, j))  # Ativa a edição ao clicar
...
def editar_celula(i, j):
    global df
    current_value = df.iloc[i, j]
    
    # Criar uma caixa de entrada para editar a célula
    entry = ctk.CTkEntry(frame_lista, width=130)
    entry.insert(0, current_value)
    entry.grid(row=i+1, column=j, padx=10, pady=5)

    # Função de salvar o valor editado
    def salvar_edicao():
        new_value = entry.get()
        df.iloc[i, j] = new_value  # Atualiza o DataFrame
        entry.destroy()  # Remove a caixa de entrada
        atualizar_lista(df)  # Atualiza a visualização dos dados

    # Botão de salvar alteração
    salvar_button = ctk.CTkButton(frame_lista, text="Salvar", command=salvar_edicao)
    salvar_button.grid(row=i+1, column=j+1, padx=10, pady=5)

# Função para salvar as alterações no arquivo Excel
def salvar_planilha():
    if excel_file_path and workbook:
        try:
            # Atualizar as células com os dados do DataFrame
            sheet = workbook[filtro_aba.get()]  # Seleciona a aba correspondente
            for i, row in df.iterrows():
                for j, value in enumerate(row):
                    sheet.cell(row=i+2, column=j+1, value=value)  # Atualiza as células (i+2 para pular o cabeçalho)

            # Salvar o arquivo Excel com as alterações
            workbook.save(excel_file_path)  # Salva o arquivo Excel com a formatação
...
def abrir_planilha():
    global excel_file, df, abas, excel_file_path, workbook
    excel_file, abas, excel_file_path, workbook = carregar_planilha()
    if abas:
...
botao_exibir_aba.grid(row=2, column=0, columnspan=3, padx=10, pady=10, )
...
botao_anterior.grid(row=4, column=0, padx=10, pady=10, )

botao_proxima = ctk.CTkButton(app, text="Próxima Página", command=pagina_proxima)
botao_proxima.grid(row=4, column=2, padx=10, pady=10, )

# Botão para salvar as alterações
botao_salvar = ctk.CTkButton(app, text="Salvar Alterações", command=salvar_planilha)
